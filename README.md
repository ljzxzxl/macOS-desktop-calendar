...
桌面小组件的每次点击都要经过系统服务转发给扩展，由扩展重新渲染整块组件后才会显示，这是 WidgetKit 的机制，无法做到普通 App 那样即时响应。本项目已做的优化：时间线只保留一个条目、点击时不等待网络（只用本地缓存）、上下月日期只画数字。

点击后组件内容会先变淡表示正在更新（`invalidatableContent`），渲染完成后恢复，以便获得即时反馈。
